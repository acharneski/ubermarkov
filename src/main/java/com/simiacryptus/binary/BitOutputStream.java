...
public class BitOutputStream
{
  ...
  public synchronized void write(final Bits encode) throws IOException
  {
    ...
    final Bits toWrite = newRemainder.range(0, newRemainder.bitLength - newRemainingBits);
  ...
  public void writeBoundedLong(final long value, final long max) throws IOException
  {
  ...
  public void writeVarLong(final long value) throws IOException
  {
    ...
    int type = Arrays.binarySearch(BitOutputStream.varLongDepths, bitLength);
    if (type < 0)
    {
      type = -type - 1;
    }
    this.write(new Bits(type, 2));
    this.write(new Bits(value, BitOutputStream.varLongDepths[type]));
  }
