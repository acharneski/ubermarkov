...
public class BitInputStream
{
  ...
  {
    final Bits checkBits = read(bits.bitLength);
    if (!bits.equals(checkBits)) throw new IOException(String.format("Check failed: %s != %s", checkBits, bits));

  }

  public void expect(final double expectedValue) throws IOException
  {
    final Bits checkBits = read(64);
    final double checkValue = Double.longBitsToDouble(checkBits.toLong());
    if (checkValue != expectedValue)
    {
      final Bits expectedBits = new Bits(Double.doubleToLongBits(expectedValue), 64);
  ...
  public long readBoundedLong(final long max) throws IOException
  {
    ...
    return 0 < bits ? read(bits).toLong() : 0;
  }

  public long readVarLong() throws IOException
  {
    final int type = (int) read(2).toLong();
    return read(BitOutputStream.varLongDepths[type]).toLong();
  }
