...
public class Bits implements Comparable<Bits>
{
  ...
  {
    this(data, Bits.highestOneBit(data));
  }
  ...
  public Bits(final long value, final int length)
  {
    ...
    if (0 > length) throw new IllegalArgumentException();
    final byte highestOneBit = Bits.highestOneBit(value);
  ...
  public Bits bitwiseAnd(final Bits right)
  {
    ...
    if (lengthDifference < 0) return concatenate(new Bits(0l, -lengthDifference)).bitwiseAnd(right);
    if (lengthDifference > 0) return bitwiseAnd(right.concatenate(new Bits(0l, lengthDifference)));
  ...
  {
    final int lengthDifference = this.bitLength - right.bitLength;
    if (lengthDifference < 0) return concatenate(new Bits(0l, -lengthDifference)).bitwiseOr(right);
    if (lengthDifference > 0) return bitwiseOr(right.concatenate(new Bits(0l, lengthDifference)));
    final Bits returnValue = new Bits(new byte[this.bytes.length], this.bitLength);
    for (int i = 0; i < this.bytes.length; i++)
    {
      returnValue.bytes[i] = this.bytes[i];
  ...
  {
    final int lengthDifference = this.bitLength - right.bitLength;
    if (lengthDifference < 0) return concatenate(new Bits(0l, -lengthDifference)).bitwiseXor(right);
    if (lengthDifference > 0) return bitwiseXor(right.concatenate(new Bits(0l, lengthDifference)));
    final Bits returnValue = new Bits(new byte[this.bytes.length], this.bitLength);
    for (int i = 0; i < this.bytes.length; i++)
    {
      returnValue.bytes[i] = this.bytes[i];
  ...
  {
    return concatenate(new Bits(0l, bits));
  }
  ...
  public Bits next()
  {
    if (!toBitString().contains("0")) return null;
    return new Bits(toLong() + 1, this.bitLength);
  }
  ...
  public String toBitString()
  {
    ...
    final byte[] shiftRight = Bits.shiftRight(this.bytes, shift);
  ...
  {
    long value = 0;
    for (final byte b : Bits.shiftRight(this.bytes, this.bytes.length * 8 - this.bitLength))
    {
