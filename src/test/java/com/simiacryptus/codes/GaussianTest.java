...
public class GaussianTest
{
  ...
  {
    final byte[] serializedData = encode(gaussian, max, value);
    final long decoded = decode(gaussian, max, serializedData);
  ...
  {
    LOG.d("T: %s", this.test(new Gaussian(100, 3), 255));
    LOG.d("T: %s", this.test(new Gaussian(100, 10), 255));
    LOG.d("T: %s", this.test(new Gaussian(100, 200), 255));
    LOG.d("T: %s", this.test(new Gaussian(100, 500), 255));
    LOG.d("T: %s", this.test(new Gaussian(500, 10), 255));
    LOG.d("T: %s", this.test(new Gaussian(-100, 10), 255));
    LOG.d("T: %s", this.test(Gaussian.fromBinomial(0.7, 3), 3));
    LOG.d("T: %s", this.test(Gaussian.fromBinomial(0.5, 1), 1));
  ...
  public void testZeros() throws IOException
  {
    for (int value = 0; value <= 0; value++)
    {
      ...
      final byte[] serializedData = encode(gaussian, 0, 0);
      Assert.assertEquals(0, serializedData.length);
      final long decoded = decode(gaussian, 0, serializedData);
      Assert.assertEquals(value, decoded);
    }
  }
}
